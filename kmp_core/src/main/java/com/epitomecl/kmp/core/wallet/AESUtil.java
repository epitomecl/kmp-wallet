...
//    private static Logger mLogger = LoggerFactory.getLogger(AESUtil.class);
    ...
    public static String decrypt(String ciphertext, String password, int iterations) throws UnsupportedEncodingException, InvalidCipherTextException, Exception {

        return decryptWithSetMode(ciphertext, password, iterations, MODE_CBC, new ISO10126d2Padding());
    }

    public static String decryptWithSetMode(String ciphertext, String password, int iterations, int mode, @Nullable BlockCipherPadding padding) throws InvalidCipherTextException, UnsupportedEncodingException, Exception {
        ...
        if (result.isEmpty()) {
            throw new Exception("Decrypted string is empty.");
        }
